...
//const dbmodels = require('../sfc/../models');
...
*/
  ...
  static nationality(direction, originalCode) {
    ...
    if (direction == BUDI.TO_ASC) {
      ...
      const foundItems = fixedMapping.filter(thisNationality => thisNationality.ASC == originalCode)
      var found=foundItems.map((item) => { return item.BUDI; });
...
//    return found ? found.BUDI : null;
  ...
  static country(direction, originalCode) {
    ...
    if (direction == BUDI.TO_ASC) {
      ...
      const foundItems = fixedMapping.filter(thisCountry => thisCountry.ASC == originalCode)
      var found=foundItems.map((item) => { return item.BUDI; });
      return  found? found.toString() : null;
...
//      return found ? found.BUDI : null;
  ...
  static qualifications(direction, originalCode) {
    ...
    if (direction == BUDI.TO_ASC) {
      ...
      return found.toString() ? found.ASC : null;
    } else {
        const foundItems = fixedMapping.filter(thisQualification => thisQualification.ASC == originalCode)
        var found=foundItems.map((item) => { return item.BUDI; });
        return  found? found.toString() : null;
//        const found = fixedMapping.find(thisQualification => thisQualification.ASC == originalCode)
//        return found.toString() ? found.BUDI : null;
        }
  }
